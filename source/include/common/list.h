...
#else
#define container_of(ptr, type, member) (           \
    (type *)((u32)ptr - OFFSETOF(type, member)))
#endif
...
{
    if((NULL == next->prev) || (NULL == next->next))
    {
        while(1);
    }
    next->prev = prev;
...
{
    return (list->next==head) ? TRUE:FALSE;
}
...
static INLINE bool list_empty_careful(const struct list_head *head)
{
    ...
    return ((next == head) && (next == head->prev)) ? TRUE:FALSE;
}
